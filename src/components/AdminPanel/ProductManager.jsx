...
import { FiTrash2, FiEdit, FiPlus, FiSave, FiImage, FiDollarSign } from 'react-icons/fi';
...
const ProductManager = () => {
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-amber-100 p-6">
      <h3 className="text-lg font-medium text-amber-900 mb-4">Manage Products</h3>
      
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
        <div>
          <label className="block text-sm font-medium text-amber-800 mb-1">Product Title</label>
          <input
            type="text"
            name="title"
        ...
        <div>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  onChange={handleInputChange}
                  className="w-full pl-8 pr-4 py-2 border border-amber-200 rounded-lg"
                  placeholder="249"
                />
              </div>
            </div>
            <div>
              <label className="block text-sm font-medium text-amber-800 mb-1">Original Price ($)</label>
              <div className="relative">
                <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                  <FiDollarSign className="text-amber-600" />
                </div>
          ...
          <label className="block text-sm font-medium text-amber-800 mb-1">Stock Quantity</label>
          <input
            type="number"
            name="stock"
          ...
          <div className="flex items-center mb-3">
            ...
            />
            <label htmlFor="isNew" className="ml-2 block text-sm text-amber-800">
              Mark as New Arrival
            </label>
          </div>
          
          <div className="mb-3">
            <label className="block text-sm font-medium text-amber-800 mb-1">Add Image URL</label>
            <div className="flex gap-2">
              ...
              <button
                ...
                className="px-3 py-2 bg-amber-700 hover:bg-amber-800 text-cream-50 rounded-lg"
              >
                <FiPlus />
              </button>
            </div>
              ...
              <tr key={product.id}>
                <td className="px-6 py-4 whitespace-nowrap">
                  <div className="flex items-center">
                    ...
                    <div className="ml-4">
                      <div className="text-sm font-medium text-amber-900">{product.title}</div>
                      <div className="text-sm text-amber-800/80 line-clamp-1">{product.description}</div>
                    </div>
                  </div>
                </td>
                <td className="px-6 py-4 whitespace-nowrap text-sm text-amber-900">
                  ${product.price} {product.originalPrice > product.price && (
                    <span className="text-xs text-amber-600 line-through">${product.originalPrice}</span>
                  )}
                </td>
                <td className="px-6 py-4 whitespace-nowrap text-sm text-amber-900">
                  {product.stock}
